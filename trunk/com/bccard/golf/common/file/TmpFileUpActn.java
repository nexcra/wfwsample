...
* @version 2009.4.8 
******************************************************************************/
public class TmpFileUpActn extends AbstractAction {
	...
	public ActionResponse execute(WaContext	context, HttpServletRequest	request, HttpServletResponse response) throws ServletException,	IOException, BaseException {
		try	{
			...
			String atcTmpPath		= AppConfig.getAppProperty("UPLOAD_TMP_PATH"); 	
			//보완검수 2009.9.15
			atcTmpPath = atcTmpPath.replaceAll("\\.\\.","");	
			
			//debug("strDOC_ROOT_PATH => " + strDOC_ROOT_PATH + "/" + atcTmpPath);		
			...
			// 저장할 파일 이름을 바꿔준다.
			int	intMaxSize	= 10*1024*1024*50; //2MB*50
			...
			if ( fsize > 1024 ) {
				String ffsize = ""+fsize+"";
				BigDecimal blen = new BigDecimal(ffsize);
			...
			request.setAttribute("varFileType"	 ,multi.getParameter("varFileType"));
