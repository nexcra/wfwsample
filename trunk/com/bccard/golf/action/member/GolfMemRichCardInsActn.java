...
public class GolfMemRichCardInsActn extends GolfActn{
	...
	public static final String TITLE = "가입 > 리치카드 등록처리";
		...
		try {
			...
			dataSet.setString("CSTMR_ID", userId);
				...
				if("Y".equals(accede))
				{
					...
					System.out.print("## GolfMemRichCardInsActn VIP카드 : userId : "+userId+"\n");
					...
					{
						dataSet.setString("strCode", "0017");	// Rich&Rich 등급으로 입력
						...
						debug("## GolfMemRichCardInsActn | 회원정보 DB 입력 처리 시작");
						addMemResult = mem_proc.execute(context, dataSet, request);	
						debug("## GolfMemRichCardInsActn | 회원정보 DB 입력 처리 결과 addMemResult : "+addMemResult);
						...
						if (addMemResult == 1) {
							...
							try { 
								...
								String smsClss = "674";
								String message = "골프라운지 Rich&Rich회원으로 정상적으로 가입되셨습니다. 감사합니다."; 
					...
					else
					{
						debug("리치카드가 없음");
						request.setAttribute("returnUrl", "GolfMemRichCardJoin.do");
						request.setAttribute("resultMsg", "Rich&Rich 카드가 없습니다.");
						strResultCode = "77";
					}
			...
			System.out.print("## GolfMemRichCardInsActn 리치카드 등록처리 : userId : "+userId+" | strResultCode : "+strResultCode+" \n");
