...
*   클래스명  : GolfAdmBusPeopleUpdActn
*   작성자    : (주)미디어포스 권영만
*   내용      : 관리자 > 이벤트->골프장버스운행이벤트->신청관리 수정 처리
*   적용범위  : Golf
*   작성일자  : 2009-09-28
************************** 수정이력 ****************************************************************
*    일자      버전   작성자   변경사항
...
* @version	1.0
******************************************************************************/
public class GolfAdmBusPeopleUpdActn extends AbstractAction {

	public static final String TITLE = "관리자 > 이벤트->골프장버스운행이벤트->신청관리 수정 처리";
			...
			BaseException
	{
		...
		try {
			...
			String mode							= parser.getParameter("mode", "upd");					// 처리구분									
			...
			input.setString("golf_cmmn_codes", 	parser.getParameter("golf_cmmn_codes",""));	
			input.setString("teof_date", 		parser.getParameter("teof_date",""));	
			input.setString("dtl_addr", 		parser.getParameter("dtl_addr",""));	
			
			// DB 처리			
			result = con.execute("admin.event.golfbus.GolfAdmBusPeopleUpdDaoProc",input);								
