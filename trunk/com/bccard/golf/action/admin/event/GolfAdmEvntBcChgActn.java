...
import com.bccard.golf.dbtao.proc.admin.event.GolfAdmEvntBcUpdDaoProc;
...
* @version	1.0
******************************************************************************/
public class GolfAdmEvntBcChgActn extends GolfActn{
	...
	***************************************************************************************/
	...
	public ActionResponse execute(	WaContext context,	HttpServletRequest request,	HttpServletResponse response) throws IOException, ServletException, BaseException {
		...
		try {
			...
			String orgImg_nm = parser.getParameter("orgImg_nm", "");	// 기존 썸네일이미지
			String bltn_strt_date = parser.getParameter("bltn_strt_date", "");	// 이벤트게시시작일자
			String bltn_end_date = parser.getParameter("bltn_end_date", "");	// 이벤트게시종료일자
			...
			// 04.실제 테이블(Proc) 조회
			GolfAdmEvntBcUpdDaoProc proc = (GolfAdmEvntBcUpdDaoProc)context.getProc("GolfAdmEvntBcUpdDaoProc");
	        ...
	        if (editResult == 1) {
				...
				//========================== 파일 업로드 End =============================================================//
				request.setAttribute("returnUrl", "admEvntBcChgForm.do");
				request.setAttribute("resultMsg", "BC Golf 이벤트 수정이 정상적으로 처리 되었습니다.");      	
	        } else {
				//========================== 파일 업로드 Start =============================================================//
				if ( !GolfUtil.isNull(img_nm)) {
				...
				//========================== 파일 업로드 End =============================================================//
				request.setAttribute("returnUrl", "admEvntBcChgForm.do");
				request.setAttribute("resultMsg", "BC Golf 이벤트 수정이 정상적으로 처리 되지 않았습니다.\\n\\n반복적으로 수정되지 않을 경우 관리자에 문의하십시오.");		        		
	        }
