...
*   클래스명  : GolfAdmEvntBcWinChgActn
*   작성자    : (주)만세커뮤니케이션 김태완
*   내용      : 관리자 BC GOLF 이벤트 당첨자 수정 처리
*   적용범위  : golf
*   작성일자  : 2009-05-27
************************** 수정이력 ****************************************************************
*    일자      버전   작성자   변경사항
...
import com.bccard.golf.dbtao.proc.admin.event.GolfAdmEvntBcWinUpdDaoProc;
...
* @version	1.0
******************************************************************************/
public class GolfAdmEvntBcWinChgActn extends GolfActn{
	
	public static final String TITLE = "관리자 BC GOLF 이벤트 당첨자 수정 처리";
	...
	***************************************************************************************/
	...
	public ActionResponse execute(	WaContext context,	HttpServletRequest request,	HttpServletResponse response) throws IOException, ServletException, BaseException {
		...
		try {
			...
			// 04.실제 테이블(Proc) 조회
			GolfAdmEvntBcWinUpdDaoProc proc = (GolfAdmEvntBcWinUpdDaoProc)context.getProc("GolfAdmEvntBcWinUpdDaoProc");
	        ...
	        if (addResult == 1) {
				...
				request.setAttribute("resultMsg", "BC GOLF 이벤트 당첨자 수정이 정상적으로 처리 되었습니다.");      	
	        } else {
				request.setAttribute("returnUrl", "admEvntBcWinChgForm.do");
				request.setAttribute("resultMsg", "BC GOLF 이벤트 당첨자 수정이 정상적으로 처리 되지 않았습니다.\\n\\n반복적으로 등록되지 않을 경우 관리자에 문의하십시오.");		        		
	        }
