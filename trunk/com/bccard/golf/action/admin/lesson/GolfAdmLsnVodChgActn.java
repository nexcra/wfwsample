...
* @version	1.0
******************************************************************************/
public class GolfAdmLsnVodChgActn extends GolfActn{
	...
	***************************************************************************************/
	...
	public ActionResponse execute(	WaContext context,	HttpServletRequest request,	HttpServletResponse response) throws IOException, ServletException, BaseException {
		...
		try {
			...
			if ( !GolfUtil.isNull(vod_nm)) {
                File tmp = new File(tmpPath,vod_nm);
                if ( tmp.exists() ) {
                    ...
                    String name = vod_nm.substring(0, vod_nm.lastIndexOf('.'));
                    String ext = vod_nm.substring(vod_nm.lastIndexOf('.'));

                    File listAttch = new File(createPath, vod_nm);
                    int i=0;
	        ...
	        if (editResult == 1) {
				...
				//========================== 파일 업로드 End =============================================================//
				request.setAttribute("returnUrl", "admLsnVodList.do");
				request.setAttribute("resultMsg", "레슨 동영상 수정이 정상적으로 처리 되었습니다.");      	
	        } else {
				//========================== 파일 업로드 Start =============================================================//
				if ( !GolfUtil.isNull(vod_nm)) {
                    File tmpAttch = new File(tmpPath, vod_nm);
	                if ( tmpAttch.exists() ) {
	                	tmpAttch.delete();
	                }				
	                
                    File realAttch = new File(realPath + subDir, vod_nm);
	                if ( realAttch.exists() ) {
	                	realAttch.delete();
	                }				
				}
				...
				//========================== 파일 업로드 End =============================================================//
				request.setAttribute("returnUrl", "admLsnVodChgForm.do");
				request.setAttribute("resultMsg", "레슨 동영상 수정이 정상적으로 처리 되지 않았습니다.\\n\\n반복적으로 수정되지 않을 경우 관리자에 문의하십시오.");		        		
	        }
