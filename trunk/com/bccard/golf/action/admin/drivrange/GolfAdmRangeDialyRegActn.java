	...
	***************************************************************************************/
	...
	public ActionResponse execute(	WaContext context,	HttpServletRequest request,	HttpServletResponse response) throws IOException, ServletException, BaseException {
		...
		try {
			...
			String rsvt_date = parser.getParameter("rsvt_date", "");  // 날짜
			String sls_end_yn = parser.getParameter("sls_end_yn", "");  // 휴장여부
			String holy_yn = parser.getParameter("holy_yn", "");  // 공휴일
			...
			String rsvt_total_num = parser.getParameter("rsvt_total_num", "");	// 예약제한인원
			String sch_gr = parser.getParameter("SCH_GR_SEQ_NO", "");	// 골프장
	        ...
	        if (addResult == 1) {
				...
				request.setAttribute("resultMsg", "드림 골프레인지 일정 등록이 정상적으로 처리 되었습니다.");      	
	        } else if (addResult == 9) {
				request.setAttribute("returnUrl", "admRangeDialyRegForm.do");
				request.setAttribute("resultMsg", "이미 등록되어있는 드림 골프레인지 일정입니다.");      		        	
	        } else {
				request.setAttribute("returnUrl", "admRangeDialyRegForm.do");
				request.setAttribute("resultMsg", "드림 골프레인지 일정 등록이 정상적으로 처리 되지 않았습니다.\\n\\n반복적으로 등록되지 않을 경우 관리자에 문의하십시오.");		        		
	        }
