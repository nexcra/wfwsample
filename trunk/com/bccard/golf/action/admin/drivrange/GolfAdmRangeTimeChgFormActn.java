...
*   클래스명  : GolfAdmRangeTimeChgFormActn
*   작성자    : (주)만세커뮤니케이션 엄지환
...
* @version	1.0
******************************************************************************/
public class GolfAdmRangeTimeChgFormActn extends GolfActn{
	
	public static final String TITLE = "관리자 드림 골프레인지 일정시간 수정 폼";
	...
	***************************************************************************************/
	...
	public ActionResponse execute(	WaContext context,	HttpServletRequest request,	HttpServletResponse response) throws IOException, ServletException, BaseException {
		...
		try {
			...
			DbTaoResult rangetimeInq = proc.execute(context, dataSet);
			
			// 05. Return 값 세팅			
			//debug("lessonInq.size() ::> " + lessonInq.size());
			
			request.setAttribute("rangedialyInqResult", resultMap);
			request.setAttribute("rangetimeInqResult", rangetimeInq);	
