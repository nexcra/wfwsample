	...
	***************************************************************************************/
	...
	public ActionResponse execute(	WaContext context,	HttpServletRequest request,	HttpServletResponse response) throws IOException, ServletException, BaseException {
		...
		try {
			...
			String holy_yn = parser.getParameter("holy_yn", "");	// 공휴일여부
			long rsvt_total_num = parser.getLongParameter("rsvt_total_num", 0L);	// 예약제한인원
			
			String[] rsvttime_sql_no = parser.getParameterValues("rsvttime_sql_no", "");	// 일정시간 번호
	        ...
	        if (editResult == 1) {
				...
				request.setAttribute("resultMsg", "드림 골프레인지 일정 수정이 정상적으로 처리 되었습니다.");      	
	        } else {
				request.setAttribute("returnUrl", "admRangeTimeChgForm.do");
				request.setAttribute("resultMsg", "드림 골프레인지 일정 수정이 정상적으로 처리 되지 않았습니다.\\n\\n반복적으로 수정되지 않을 경우 관리자에 문의하십시오.");		        		
	        }
