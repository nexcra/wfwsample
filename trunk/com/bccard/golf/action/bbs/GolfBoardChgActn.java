...
import com.bccard.golf.dbtao.proc.bbs.GolfBoardUpdDaoProc;
...
* @version	1.0
******************************************************************************/
public class GolfBoardChgActn extends GolfActn{
	...
	***************************************************************************************/
	...
	public ActionResponse execute(	WaContext context,	HttpServletRequest request,	HttpServletResponse response) throws IOException, ServletException, BaseException {
		...
		String listUrl = super.getActionParam(context, "listUrl");
		...
		try {
			...
			String hd_yn = parser.getParameter("hd_yn", "");
			...
			String orgFile_nm = parser.getParameter("orgFile_nm", "");	// 기존 이미지

			if (!id.trim().equals(userId)) {
				subpage_key = "errReUrl";
				request.setAttribute("returnUrl", reUrl);
				request.setAttribute("resultMsg", "본인이 작성한 글만 수정이 가능합니다.");   				
			} else {	
		        ...
		        if (editResult == 1) {
		        	//========================== 파일 업로드 Start =============================================================//
					if ( !GolfUtil.isNull(file_nm) && !GolfUtil.isNull(file_nm)) {
	                    File realAttch = new File(realPath + subDir, orgFile_nm);
		                if ( realAttch.exists() ) {
		                	realAttch.delete();
		                }				
					}
					...
					request.setAttribute("returnUrl", reUrl);
					request.setAttribute("resultMsg", "게시물 수정이 정상적으로 처리 되었습니다.");      	
		        } else {
		        	//========================== 파일 업로드 Start =============================================================//
					if ( !GolfUtil.isNull(file_nm)) {
					...
					request.setAttribute("resultMsg", "게시물 수정이 정상적으로 처리 되지 않았습니다.\\n\\n반복적으로 수정되지 않을 경우 관리자에 문의하십시오.");		        		
		        }
				// 05. Return 값 세팅			
