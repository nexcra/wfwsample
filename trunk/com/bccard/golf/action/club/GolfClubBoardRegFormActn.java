...
* @version	1.0
******************************************************************************/
public class GolfClubBoardRegFormActn extends GolfActn{
	...
	***************************************************************************************/
	...
	public ActionResponse execute(	WaContext context,	HttpServletRequest request,	HttpServletResponse response) throws IOException, ServletException, BaseException {
		...
		try {
			...
			// Request 값 저장			
			String sch_club_seq_no		= parser.getParameter("club_code", "");
			String sch_bbrd_seq_no		= parser.getParameter("bbs_code", "");
			
			paramMap.put("sch_club_seq_no", sch_club_seq_no);
			paramMap.put("sch_bbrd_seq_no", sch_bbrd_seq_no);
						
			// 03.Proc 에 던질 값 세팅 (Proc에 dataSet 형태의 배열(?)로 request값 또는 조회값을 던진다.)
			DbTaoDataSet dataSet = new DbTaoDataSet(TITLE);
			dataSet.setString("SCH_CLUB_SEQ_NO", sch_club_seq_no);
			dataSet.setString("SCH_BBRD_SEQ_NO", sch_bbrd_seq_no);
