...
* @version	1.0
******************************************************************************/
public class GolfManiaPagChgActn extends GolfActn{
	...
	***************************************************************************************/
	...
	public ActionResponse execute(	WaContext context,	HttpServletRequest request,	HttpServletResponse response) throws IOException, ServletException, BaseException {
		...
		try { 
			...
			String car_type_code= parser.getParameter("car_type_code", "");		// 차량
	        ...
	        if (editResult == 1) {
	        	...
	        	if (scoop_cp_cd.equals("0003")) {
	        		...
	        		request.setAttribute("returnUrl", "/app/golf/admMagazineChgForm.do");
	        		request.setAttribute("resultMsg", "잡지구독할인 신청 프로그램 수정이 정상적으로 처리 되었습니다."); 	

	        	}else{
	        		if (!GolfUtil.isNull(prize_yn)) { 	//진행여부 (진행/취소) 수정시
	        			request.setAttribute("returnUrl", "/app/golf/admManiaList.do");
	        			
	        		}
	        		else{								//상담여부 (완료/대기) 수정시
	        			request.setAttribute("returnUrl", "/app/golf/admManiaChgForm.do");
	        		}
	        		request.setAttribute("resultMsg", "리무진할인 신청 프로그램 수정이 정상적으로 처리 되었습니다."); 
	        	}
	        
	        } else {
				request.setAttribute("returnUrl", "/app/golf/admManiaInq.do");
