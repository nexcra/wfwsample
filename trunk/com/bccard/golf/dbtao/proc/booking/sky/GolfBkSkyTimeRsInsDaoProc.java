	...
	public DbTaoResult execute(WaContext context, TaoDataSet data,	HttpServletRequest request) throws BaseException  {
		...
		try {
			...
			if (recvOverLapChk == 0) {
				...
				debug("=================GolfBkParTimeRsInsDaoProc============= 01. 예약등록 처리");
	            // 01. insert
				...
				debug("=================GolfBkSkyTimeRsInsDaoProc============= 02. 예약이며 사이버 머니를 사용했을 경우 사이버 머니를 차감한다.");
				if(payType.equals("cyber")){
	
					debug("=================GolfBkSkyTimeRsInsDaoProc=============  02-0. 현재 사이버 머니 금액을 가져온다.");
	    			...
	    			if(rs != null) rs.close();
	                if(pstmt != null) pstmt.close();
					debug("=================GolfBkSkyTimeRsInsDaoProc=============  02-0. 현재 사이버 머니 금액을 가져온다.-1");
	                
	            	// 02-1. 사이버머니 사용내역 테이블에 금액을 넣어준다.
	    			debug("=================GolfBkSkyTimeRsInsDaoProc============= 02-1. 사이버머니 사용내역 테이블에 금액을 넣어준다.");
	                //**SEQ_NO 가져오기*******************************************************
	    			...
	    			pstmt3.setString(++idx, "0005" );						//CBMO_USE_CLSS :  0004:파3부킹
	    			...
	    			debug("=================GolfBkSkyTimeRsInsDaoProc============= 02-2. 회원테이블에 사이버머니 내역을 업데이트 해준다.");
