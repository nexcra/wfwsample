...
public class GolfRoundAfterListProc extends AbstractProc {

	private static GolfRoundAfterListProc instance = null;
	static{
		synchronized(GolfRoundAfterListProc.class){
			if(instance == null){
				instance = new GolfRoundAfterListProc();				
			}
		}
	}
	public static GolfRoundAfterListProc getInstance(){
		return instance;
	}
	...
	{
		String title = dataSet.getString("TITLE1");
		ResultSet rs = null;
		Connection conn = null;
		PreparedStatement pstmt = null;
		DbTaoResult result1 = null;
		int idx = 0;
		String vsearch_word = "";
		...
		try
		{
			...
			result1 =  new DbTaoResult(title);
			if(rs != null){
				while(rs.next()){

					if(!existsData){
						result1.addString("RESULT", "00");
						record_cnt =rs.getLong("RECORD_CNT");
					}
					result1.addLong("BOARD_NO", rs.getLong("BOARD_NO"));
					...
					result1.addString("BOARD_SUBJ", subj);					
					result1.addString("LIST_INQ_CLSS", rs.getString("LIST_INQ_CLSS"));
					result1.addString("LIST_IMG_PATH", rs.getString("LIST_IMG_PATH"));
	                result1.addLong("SORT_KEY", rs.getLong("SORT_KEY"));
	                result1.addLong("REF_NO", rs.getLong("REF_NO"));
	                result1.addLong("ANS_STG", rs.getLong("ANS_STG"));
	                result1.addLong("ANS_LEV", rs.getLong("ANS_LEV"));
	                result1.addString("REG_DATE", rs.getString("REG_DATE"));
	                result1.addString("REG_NM", rs.getString("REG_NM"));
	                result1.addString("REG_ACCOUNT", rs.getString("REG_ACCOUNT"));
					result1.addString("REG_IP", rs.getString("REG_IP"));
					result1.addLong("RECORD_CNT", rs.getLong("RECORD_CNT"));
	                result1.addString("REG_DATE2", rs.getString("REG_DATE2"));
	                result1.addString("TODAY", rs.getString("TODAY"));
	                result1.addString("HOT_INFO_YN", rs.getString("HOT_INFO_YN"));
	                result1.addString("ATC_FILE_YN", rs.getString("ATC_FILE_YN"));
	                result1.addLong("ROW_NUM", record_cnt - (recordsInPage * (pageNo-1) + (i++)) + 1);
					result1.addLong("PAGENO", pageNo);
					result1.addString("NEWYN" , rs.getString("NEWYN"));
					if(board_cd.equals("12")){
						result1.addString("SCOR_APPL", rs.getString("SCOR_APPL2"));
					}else{
						result1.addString("SCOR_APPL", rs.getString("SCOR_APPL")); 
					}
					result1.addLong("READ_CNT", rs.getLong("READ_CNT"));
					if(!board_cd.equals("12")){
						result1.addInt("AN_CNT", rs.getInt("AN_CNT"));
					}
					
					// 댓글 개수
					if ( "Y".equals(add_yn) ) {
						result1.addInt("ADD_CNT", rs.getInt("ADD_CNT"));
					}
					if(rs.getString("SCOR_APPL_YN") == null){
	                	result1.addString("SCOR_APPL_YN", "0");
	                }else{
	                	result1.addString("SCOR_APPL_YN", rs.getString("SCOR_APPL_YN"));
	                }
					existsData = true;
	...
	private StringBuffer getRoundingListSelect(TaoDataSet data) throws Exception
	{
		...
		if(!board_cd.equals("12")){
			sb.append("			,(SELECT COUNT(BOARD_NO) FROM BCDBA.TBGFBOARD WHERE BOARD_CD = ? AND REF_NO = X.REFNO) AN_CNT	\n");
		}
		if(board_cd.equals("12")){
			sb.append("	,CASE WHEN X.BOARD_CD = '12' AND X.ANS_LEV = 1	\n");
			sb.append("		THEN (SELECT SCOR_APPL_YN FROM BCDBA.TBGFBOARD WHERE BOARD_NO = X.REF_NO) 	\n");
			sb.append("		ELSE X.SCOR_APPL END SCOR_APPL2	\n");
		}
		sb.append("		FROM(	\n");
		sb.append("			SELECT 	\n");
		sb.append("				A.BOARD_NO, A.BOARD_CD, A.BOARD_SUBJ, A.BOARD_TEXT, A.HOT_INFO_YN, A.READ_CNT, SCOR_APPL_YN, A.LIST_INQ_CLSS, A.LIST_IMG_PATH,	\n");
