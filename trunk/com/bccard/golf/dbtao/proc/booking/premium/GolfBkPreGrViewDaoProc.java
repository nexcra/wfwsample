 ...
 * @version	1.0
 ******************************************************************************/
public class GolfBkPreGrViewDaoProc extends AbstractProc {
	
	/** *****************************************************************
	 * GolfBkPreGrViewDaoProc 프로세스 생성자
	 * @param N/A
	 ***************************************************************** */
	public GolfBkPreGrViewDaoProc() {}	
		...
		try {
			...
			if(rs != null) {
				while(rs.next())  {
					...
					reader = rs.getCharacterStream("CURS_INTD_CTNT");
					if( reader != null )  {
					...
					result.addString("CURS_INTD_CTNT", bufferSt.toString());	

					reader = null;
					bufferSt = new StringBuffer();
					reader = rs.getCharacterStream("GREEN_CHRG_INFO");
					if( reader != null )  {
						char[] buffer = new char[1024]; 
						int byteRead; 
						while((byteRead=reader.read(buffer,0,1024))!=-1) 
					...
					result.addString("GREEN_CHRG_INFO", bufferSt.toString());	
	...
	// 주말부킹 사용 여부 가져오기
	public DbTaoResult execute_weekend(WaContext context, TaoDataSet data) throws BaseException {
		...
		try {
			...
			//조회 ----------------------------------------------------------			
			String sql = this.getWeekendQuery();   
    ...
    ************************************************************************ */ 
    private String getSelectQuery(){
		...
		sql.append("\t  CURS_INTD_CTNT, GREEN_CHRG_INFO																									\n");
		sql.append("\n FROM");
		sql.append("\n BCDBA.TBGAFFIGREEN");
	...
	/** ***********************************************************************
     * Query를 생성하여 리턴한다. - 주말부킹 사용여부 
     ************************************************************************ */ 
     private String getWeekendQuery(){
         StringBuffer sql = new StringBuffer();

 		sql.append("\n");
 		sql.append("\t SELECT CO_NM	\n");
