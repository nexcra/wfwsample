...
public class GolfManiaListDaoProc extends AbstractProc {
		...
		try {
			...
			if(rs != null) {			 
				...
				while(rs.next())  {	
					...
					result.addString("STTL_STAT_CLSS" 		,rs.getString("STTL_STAT_CLSS") );			// 결제여부
    ...
    ************************************************************************ */
    private String getSelectQuery(String search_sel, String search_word, String sckd_code, String scnsl_yn, String sprgs_yn, String search_dt1, String search_dt2, String scoop_cp_cd){
		...
		sql.append("\n  (SELECT CAR_KND_NM FROM BCDBA.TBGAMTMGMT  WHERE CAR_KND_CLSS=TGL.GOLF_LMS_CAR_KND_CLSS) AS CAR_KND_NM,   ");			//차종
		sql.append("\n  (SELECT NORM_PRIC FROM BCDBA.TBGAMTMGMT  WHERE CAR_KND_CLSS=TGL.GOLF_LMS_CAR_KND_CLSS) AS NORM_PRIC,   ");			//차량별 금액
		sql.append("\n  (SELECT PCT20_DC_PRIC FROM BCDBA.TBGAMTMGMT  WHERE CAR_KND_CLSS=TGL.GOLF_LMS_CAR_KND_CLSS) AS PCT20_DC_PRIC,   ");	//차량별 금액 20%할인
		sql.append("\n  (SELECT PCT30_DC_PRIC FROM BCDBA.TBGAMTMGMT  WHERE CAR_KND_CLSS=TGL.GOLF_LMS_CAR_KND_CLSS) AS PCT30_DC_PRIC,   ");	//차량별 금액 30%할인
		
		sql.append("\n  (SELECT STTL_STAT_CLSS FROM BCDBA.TBGSTTLMGMT  WHERE STTL_GDS_SEQ_NO=TGL.APLC_SEQ_NO) AS STTL_STAT_CLSS   ");	//결제여부
