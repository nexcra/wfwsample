...
public class GolfEvntBnstRegDaoProc extends AbstractProc {
	...
	public int execute(WaContext context,  HttpServletRequest request ,TaoDataSet data) throws BaseException {
		...
		try {
			...
			String trm_unt						= data.getString("trm_unt");
			...
			for(int i=0; i<compn_no-1; i++){
				...
				if(compn_cdhd_grd_seq_no.length>i){
					arr_cdhd_grd_seq_no = compn_cdhd_grd_seq_no[i].replaceAll(",", "");
					
					input_cdhd_grd_seq_no = arr_cdhd_grd_seq_no.substring(0,arr_cdhd_grd_seq_no.indexOf(":"));							//CDHD_GRD_SEQ_NO
					input_sttl_amt = arr_cdhd_grd_seq_no.substring(arr_cdhd_grd_seq_no.indexOf(":")+1,arr_cdhd_grd_seq_no.length());		//STTL_AMT
				}else{
					arr_cdhd_grd_seq_no = "";
				}
				...
				pstmt.setString(idx++, input_cdhd_grd_seq_no);	//code-CDHD_GRD_SEQ_NO	
				...
				pstmt.setString(idx++, "");
				pstmt.setString(idx++, "");
				pstmt.setString(idx++, input_sttl_amt);			//cost-STTL_AMT
				result = pstmt.executeUpdate();
			}
    ...
    ************************************************************************ */
    private String getEvtQuery(){
		...
		sql.append("\n		SITE_CLSS, APLC_SEQ_NO,TRM_UNT, GOLF_SVC_APLC_CLSS, GREEN_NM, APLC_ATON, RSVT_DATE, RSV_TIME, BKG_PE_NM, CDHD_NON_CDHD_CLSS, CDHD_ID	");
		sql.append("\n	    , JUMIN_NO, CDHD_GRD_SEQ_NO, HP_DDD_NO, HP_TEL_HNO, HP_TEL_SNO, EMAIL, HADC_NUM, EVNT_PGRS_CLSS, STTL_STAT_CLSS, NOTE, CUS_RMRK	");
		sql.append("\n	) VALUES (	");
