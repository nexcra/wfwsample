...
*   클래스명  : GolfAdmEvntBcWinListDaoProc
*   작성자    : (주)만세커뮤니케이션 김태완
*   내용      : 관리자 > 이벤트 > BC GOLF 이벤트 > 당첨자 입력
*   적용범위  : golf
*   작성일자  : 2009-05-27
************************** 수정이력 ****************************************************************
*    일자      버전   작성자   변경사항
 ...
 * @version	1.0
 ******************************************************************************/
public class GolfAdmEvntBcWinListDaoProc extends AbstractProc {
	
	/** *****************************************************************
	 * GolfAdmEvntBcWinListDaoProc 프로세스 생성자
	 * @param N/A
	 ***************************************************************** */
	public GolfAdmEvntBcWinListDaoProc() {}	
	...
	public DbTaoResult execute(WaContext context,  HttpServletRequest request ,TaoDataSet data) throws BaseException {
		...
		try {
			...
			String sdisp_yn			= data.getString("SDISP_YN");
			String sbkps_sdate		= data.getString("SBKPS_SDATE");
			String sbkps_edate		= data.getString("SBKPS_EDATE");
			String sevnt_from		= data.getString("SREG_SDATE");
			String sevnt_to			= data.getString("SREG_EDATE");

			String sql = this.getSelectQuery(search_sel, search_word, sgr_nm, sdisp_yn, sbkps_sdate, sbkps_edate, sevnt_from, sevnt_to);   
			...
			if (!GolfUtil.isNull(sdisp_yn))	pstmt.setString(++idx, sdisp_yn);
    ...
    ************************************************************************ */
    ...
    private String getSelectQuery(String search_sel, String search_word, String sgr_nm, String sdisp_yn, String  sbkps_sdate, String  sbkps_edate, String  sevnt_from,  String sevnt_to){
		...
		if (!GolfUtil.isNull(sdisp_yn)) {
			sql.append("\n 				AND TGEJ.BLTN_YN = ?	");
		}
		if (!GolfUtil.isNull(sbkps_sdate) && !GolfUtil.isNull(sbkps_edate)) {
			sql.append("\n 				AND TGE.BKPS_DATE BETWEEN ? AND ?	");
		}
		if (!GolfUtil.isNull(sevnt_from) && !GolfUtil.isNull(sevnt_to)) {
