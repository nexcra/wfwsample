 ...
 * @version	1.0  
 ******************************************************************************/
public class GolfAdmMemStatsAgeDaoProc extends AbstractProc {
	 ...
	 * @param N/A
	 ***************************************************************** */
	public GolfAdmMemStatsAgeDaoProc() {}	
	 ...
	 * @return TaoResult
	 */
	public DbTaoResult executeAgeGrd(WaContext context,  HttpServletRequest request) throws BaseException {
		...
		try {
			...
			String sql = this.getAgeGrdQuery();
	...
	public DbTaoResult executeAgeLct(WaContext context,  HttpServletRequest request) throws BaseException {
		...
		try {
			conn = context.getDbConnection("default", null);
			String sql = this.getAgeLctQuery();
			pstmt = conn.prepareStatement(sql.toString());
			rs = pstmt.executeQuery();
	...
	public DbTaoResult executeAgeSex(WaContext context,  HttpServletRequest request) throws BaseException {
		...
		try {
			conn = context.getDbConnection("default", null);
			String sql = this.getAgeSexQuery();
			pstmt = conn.prepareStatement(sql.toString());
			rs = pstmt.executeQuery();
    ...
    ************************************************************************ */
    private String getAgeGrdQuery(){
		...
		sql.append("\t	FROM (	\n");
		sql.append("\t	    SELECT AGE, CNT, ROW_NUMBER() OVER(PARTITION BY AGE ORDER BY DECODE(GRD, 'Champion', 1	\n");
		sql.append("\t	    , 'Black', 2, 'Blue', 3, 'Gold', 4, 'White', 5, 'NH티타늄', 6, 'NH플래티늄', 7	\n");
		sql.append("\t	    , '골프투어멤버스', 8, '나의 알파 플래티늄', 9, '나의 알파 플래티늄', 10)) RN	\n");
		sql.append("\t	    FROM (	\n");
	...
	************************************************************************ */
	private String getAgeLctQuery(){
		...
		sql.append("\t	    , MIN(DECODE(RN,7,CNT)) GRD7	\n");
		sql.append("\t	FROM (	\n");
		sql.append("\t	    SELECT GAGE, CNT, ROW_NUMBER() OVER(PARTITION BY GAGE ORDER BY	\n");
	...
	************************************************************************ */
	private String getAgeSexQuery(){
		...
		sql.append("\t	    , MIN(DECODE(RN,2,CNT)) GRD2	\n");
		sql.append("\t	FROM (	\n");
		sql.append("\t	    SELECT GAGE, CNT, ROW_NUMBER() OVER(PARTITION BY GAGE ORDER BY SEX) RN	\n");
		sql.append("\t	    FROM (	\n");
