...
//			debug("=========GolfBkParTimeReserveActn====affi_GREEN_SEQ_NO : " + parser.getParameter("AFFI_GREEN_SEQ_NO", ""));
			...
			if (fineResult==0){
				...
				request.setAttribute("resultMsg", "해당 일자의 등록이 종료되었습니다. 다시 선택해주세요.");  
				...
				if (addResult != null && addResult.isNext()) {
					...
					if ("00".equals(sRESULT))
					{
						...
						debug("==========예약하고 예약번호를 예약결과 보기 페이지로 보내는 액션 => rsvt_SQL_NO : " + rsvt_SQL_NO);
						...
						if (!email_id.equals("")) {
							...
							emailTitle = "[Golf Loun.G] 파3 부킹이 완료되었습니다.";
						...
						if (!hp_TEL.equals("")) {
							...
							//debug("SMS시작>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>..");
							String smsClss = "637";
							String message = "[파3] "+userNm+"님 "+green_NM+" "+bk_DATE_EMAIL+" 예약완료 - Golf Loun.G";
						...
						request.setAttribute("returnUrl", "GolfBkParTimeList.do");
						request.setAttribute("resultMsg", "예약이 정상적으로 처리 되지 않았습니다. 반복적으로 수정되지 않을 경우 관리자에 문의하십시오.");		        		
					}
		        } else {
					request.setAttribute("returnUrl", "GolfBkParTimeList.do");
					request.setAttribute("resultMsg", "예약이 정상적으로 처리 되지 않았습니다. 반복적으로 수정되지 않을 경우 관리자에 문의하십시오.");		        		
