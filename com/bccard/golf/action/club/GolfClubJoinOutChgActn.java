...
import com.bccard.golf.dbtao.proc.club.GolfClubMyInfoUpdDaoProc;
...
* @version	1.0
******************************************************************************/
public class GolfClubJoinOutChgActn extends GolfActn{
	...
	***************************************************************************************/
	...
	public ActionResponse execute(	WaContext context,	HttpServletRequest request,	HttpServletResponse response) throws IOException, ServletException, BaseException {
		...
		try {
			...
			// 04.실제 테이블(Proc) 조회			
			GolfClubMyInfoUpdDaoProc proc = (GolfClubMyInfoUpdDaoProc)context.getProc("GolfClubMyInfoUpdDaoProc");
			int editResult = proc.getJoinOut(context, dataSet);	
			
	        if (editResult == 1) {
				request.setAttribute("returnUrl", reUrl);
				request.setAttribute("resultMsg", "동호회 탈퇴가 정상적으로 처리 되었습니다.");      	
	        } else {
				request.setAttribute("returnUrl", errReUrl);
				request.setAttribute("resultMsg", "동호회 탈퇴가 정상적으로 처리 되지 않았습니다.\\n\\n반복적으로 등록되지 않을 경우 관리자에 문의하십시오.");		        		
	        }
