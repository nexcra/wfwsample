...
import com.bccard.golf.dbtao.proc.admin.member.GolfLoungPAYAgentDaoProc;
 ...
 * @author
 ******************************************************************************/
public class GolfLoungPAYAgent extends AbstractAction implements Agent {
	public static final String TITLE = "골프라운지 월결제 작업"; 
	...
	public void execute(WaContext context) throws BaseException {
		...
		if ( "Y".equals(yn) ) {
			info("[골프라운지 PAY★★★ Agent Start ★★★]");
			runAgent(context);
			info("[골프라운지 PAY ★★★ Agent End ★★★]");
		}
	}
	/**
	 * @see com.bccard.waf.core.Agent#execute(com.bccard.waf.core.WaContext)
	...
	public void runAgent(WaContext context) throws BaseException {
		try {
			...
			GolfLoungPAYAgentDaoProc proc = (GolfLoungPAYAgentDaoProc)context.getProc("GolfLoungPAYAgentDaoProc");
			int fineResult = proc.execute(context);
			debug("GolfLoungPayAgent 결제 : " + fineResult + "건");
