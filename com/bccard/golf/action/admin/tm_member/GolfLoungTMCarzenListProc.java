 ...
 * @version 2009-05-06
 **************************************************************************** */
public class GolfLoungTMCarzenListProc extends AbstractProc {
	public static final String TITLE = "TM회원 조회";
	/** *****************************************************************
	 * GolfLoungTMListProc 프로세스 생성자
	 * @param N/A 
	 ***************************************************************** */
	public GolfLoungTMCarzenListProc() { 
	...
	************************************************************************ */
	public TaoResult execute(WaContext context, TaoDataSet data) throws DbTaoException  {
		...
		try {				
			...
			sql.append("\n");
			sql.append("SELECT	*  \n");
			sql.append("FROM (SELECT  ROWNUM RNUM, CEIL(ROWNUM/?) AS PAGE, A.MB_CDHD_NO,A.HG_NM, A.EMAIL_ID,  \n");
			sql.append("				A.ACPT_CHNL_CLSS,A.TB_RSLT_CLSS, A.REJ_RSON, A.JUMIN_NO, A.GOLF_CDHD_GRD_CLSS,  \n");
			sql.append("				CASE WHEN A.GOLF_CDHD_GRD_CLSS ='1' THEN '골드' WHEN A.GOLF_CDHD_GRD_CLSS ='2' THEN '블루' ELSE '챔피온' END GOLF_CDHD_GRD_CLSS_NM,  \n");
			sql.append("              COUNT(*) OVER() AS TOT_CNT, A.RECP_DATE ,A.SQ1_TCALL_DATE, A.CONC_DATE, A.JOIN_CHNL, A.AUTH_CLSS, A.CARD_NO, \n");  
			...
			sql.append("              '-'    SECE_YN, SUBSTR(A.HP_DDD_NO,2,3) HP_DDD_NO, A.HP_TEL_HNO, A.HP_TEL_SNO   \n");    
			...
			if(rs != null)
			{
				...
				while(rs.next())  
				{
					...
					result.addString("CONC_DATE",				rs.getString("CONC_DATE"));
