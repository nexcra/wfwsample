	...
	***************************************************************************************/
	...
	public ActionResponse execute(	WaContext context,	HttpServletRequest request,	HttpServletResponse response) throws IOException, ServletException, BaseException {
		...
		try {
			...
			// Request 값 저장
			String[] lsn_seq_no = parser.getParameterValues("cidx", ""); 		// 일련번호
			String view_YN = parser.getParameter("VIEW_YN", ""); 		// 상태
			...
			if (lessonDelResult == lsn_seq_no.length) {
				request.setAttribute("resultMsg", "상태처리가 정상적으로 처리 되었습니다.");	
			} else {
				request.setAttribute("resultMsg", "상태처리가 정상적으로 처리 되지 않았습니다.");
			}
