...
* @version	1.0
******************************************************************************/
public class GolfadmPreRsUpdActn extends GolfActn{
	...
	***************************************************************************************/
	...
	public ActionResponse execute(	WaContext context,	HttpServletRequest request,	HttpServletResponse response) throws IOException, ServletException, BaseException {
		...
		try {
			...
			String rsvt_ABLE_BOKG_TIME_SEQ_NO = parser.getParameter("RSVT_ABLE_BOKG_TIME_SEQ_NO", "");
			...
			dataSet.setString("USER_ID", cdhd_id);
			dataSet.setString("APPR_OPION", appr_opion);
			dataSet.setString("ADD_APPR_OPION", add_appr_opion);
			dataSet.setString("RSVT_ABLE_BOKG_TIME_SEQ_NO", rsvt_ABLE_BOKG_TIME_SEQ_NO);
			
			// 04.실제 테이블(Proc) 조회
			GolfadmPreRsUpdDaoProc proc = (GolfadmPreRsUpdDaoProc)context.getProc("GolfadmPreRsUpdDaoProc");		
			int editResult = proc.execute(context, dataSet);
						
	        if (editResult == 1) {
				request.setAttribute("returnUrl", "admPreRsList.do");
