...
*   클래스명  : GolfadmPlatinumJuminSearchActn
*   작성자    : 이정규
*   내용      : 이름으로 정보 결과
*   적용범위  : Golf
*   작성일자  : 2010-09-27
...
* @version	1.0 
******************************************************************************/
public class GolfadmPlatinumJuminSearchActn extends GolfActn{
	
	public static final String TITLE = "주민등록번호로 카드리스트 가져오기"; 
	...
	***************************************************************************************/
	...
	public ActionResponse execute(WaContext context, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException, BaseException {
		...
		try {
			...
			// Request 값 저장
			String sch_jumin = parser.getParameter("JUMINNO","");
			debug("@@@sch_jumin :"+sch_jumin);
			
			// 03.Proc 에 던질 값 세팅 (Proc에 dataSet 형태의 배열(?)로 request값 또는 조회값을 던진다.)
			DbTaoDataSet dataSet = new DbTaoDataSet(TITLE);
			//dataSet.setString("SCH_TEXT", sch_jumin);
			
			try{
				checkJoltVip(context, request,ett, sch_jumin);
			}catch (Throwable t){
