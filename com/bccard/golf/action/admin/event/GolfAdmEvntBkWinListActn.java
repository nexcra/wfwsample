...
*   클래스명  : GolfAdmEvntBkWinListActn
*   작성자    : (주)만세커뮤니케이션 김태완
*   내용      : 관리자 > 이벤트 > 프리미엄 부킹 이벤트 > 당첨자 관리 
*   적용범위  : Golf
*   작성일자  : 2009-05-27
************************** 수정이력 ****************************************************************
*    일자      버전   작성자   변경사항
...
import com.bccard.golf.dbtao.proc.admin.event.GolfAdmEvntBkWinListDaoProc;
...
* @version	1.0
******************************************************************************/
public class GolfAdmEvntBkWinListActn extends GolfActn{
	...
	***************************************************************************************/
	...
	public ActionResponse execute(WaContext context, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException, BaseException {
		...
		try {
			...
			String sdisp_yn			= parser.getParameter("sdisp_yn", "");
			...
			dataSet.setString("SDISP_YN", sdisp_yn);
			...
			dataSet.setString("EVNT_CLSS", "0002");
			
			// 04.실제 테이블(Proc) 조회
			GolfAdmEvntBkWinListDaoProc proc = (GolfAdmEvntBkWinListDaoProc)context.getProc("GolfAdmEvntBkWinListDaoProc");
			GolfadmBkTimeRegFormDaoProc proc2 = (GolfadmBkTimeRegFormDaoProc)context.getProc("admBkTimeRegFormDaoProc");
			DbTaoResult evntBkWinListResult = (DbTaoResult) proc.execute(context, request, dataSet);
			DbTaoResult titimeGreenListResult = (DbTaoResult) proc2.execute(context, request, dataSet);

			paramMap.put("resultSize", String.valueOf(evntBkWinListResult.size()));
			
			request.setAttribute("evntBkWinListResult", evntBkWinListResult);
