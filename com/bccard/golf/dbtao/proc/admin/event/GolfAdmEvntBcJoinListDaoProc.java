...
public class GolfAdmEvntBcJoinListDaoProc extends AbstractProc {
		...
		try {
			...
			if(rs != null) {			 
				...
				while(rs.next())  {	
					...
					debug("=============GolfAdmEvntBcJoinListDaoProc===========REG_ATON => " + rs.getString("REG_ATON"));
    ...
    ************************************************************************ */
    ...
    private String getSelectQuery(String search_sel, String search_word, String sevnt_seq_no, String sprize_yn, String  sevnt_from,  String sevnt_to){
		...
		sql.append("\n 				TGU.HG_NM,   	");
		sql.append("\n 				TGR.APLC_SEQ_NO, DECODE(NVL(TGR.PRZ_WIN_YN,'N'),'Y','당첨','N','미당첨') PRZ_WIN_YN, TGR.CDHD_ID, TGR.HP_DDD_NO||'-'||TGR.HP_TEL_HNO||'-'||TGR.HP_TEL_SNO HP, TO_CHAR(TO_DATE(TGR.REG_ATON, 'YYYYMMDDHH24MISS'), 'YYYY-MM-DD HH24:MI:SS') REG_ATON   	");
		sql.append("\n 				, TGR.RIDG_PERS_NUM AS TNUM, TGU.ZIPADDR||TGU.DETAILADDR AS ADDR, T4.GOLF_CMMN_CODE_NM AS MGRADE 	");
		sql.append("\n 				FROM 	");
		sql.append("\n 				BCDBA.TBGAPLCMGMT TGR, BCDBA.TBGEVNTMGMT TGE, BCDBA.TBGGOLFCDHD TGU	");
		sql.append("\n 				, BCDBA.TBGGOLFCDHDCTGOMGMT T3, BCDBA.TBGCMMNCODE T4	");
		sql.append("\n 				WHERE TGR.LESN_SEQ_NO = TGE.EVNT_SEQ_NO 	");
		sql.append("\n 				AND TGR.CDHD_ID = TGU.CDHD_ID 	");
		sql.append("\n 				AND TGR.GOLF_SVC_APLC_CLSS = '0009' 	");
		sql.append("\n 				AND TGU.CDHD_CTGO_SEQ_NO=T3.CDHD_CTGO_SEQ_NO 	");
