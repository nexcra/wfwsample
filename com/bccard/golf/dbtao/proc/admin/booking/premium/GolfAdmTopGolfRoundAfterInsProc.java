...
*   클래스명  : GolfAdmTopGolfRoundAfterInsProc
*   작성자    : shin cheong gwi
*   내용      : 관리자 > 라운딩후기
*   적용범위  : golfloung
*   작성일자  : 2010-11-20
...
public class GolfAdmTopGolfRoundAfterInsProc extends AbstractObject{

	private static GolfAdmTopGolfRoundAfterInsProc instance = null;
	static{
		synchronized(GolfAdmTopGolfRoundAfterInsProc.class){
			if(instance == null){
				instance = new GolfAdmTopGolfRoundAfterInsProc();
			}
		}
	}
	public static GolfAdmTopGolfRoundAfterInsProc getInstance(){
		return instance;
	}
	...
	public boolean execute(WaContext context,  HttpServletRequest request ,TaoDataSet dataSet) throws BaseException, SQLException 
	{
		...
		try 
		{
			...
			if(access.equals("reply")){
				if(ans_lev == 0L){
					pstmt = conn.prepareStatement(this.getRoundingReplySeqQuery().toString());
						pstmt.setInt(1, board_no);
					rs = pstmt.executeQuery();
					if(rs.next()){
						ans_stg2 = rs.getLong(1);
						ans_lev2 = rs.getLong(2);
					}
			...
			if( access.equals("reply") && board_cd.equals("12")){
				pstmt = conn.prepareStatement(this.getScorQuery().toString());
					pstmt.setInt(1, board_no);
				rs = pstmt.executeQuery();
				if(rs.next()){
			...
			if(access.equals("write")){
				boardno = seq;
				refno = seq;
			}else if(access.equals("edit")){
				boardno = board_no;
				refno = board_no;
			}else if(access.equals("reply")){
				if(board_no > 0 && ans_lev == 0L){ 
					boardno = seq;
					refno = board_no;
					ansstg = ans_stg2;
					anslev = ans_lev2;
				}else if(board_no > 0 && ans_lev > 0L){
					boardno = board_no;
					refno = board_no;
					ansstg = ans_stg;
					anslev = ans_lev;
			...
			pstmt = conn.prepareStatement(this.setRoundingInsQuery(dataSet).toString());
				...
				pstmt.setInt(++idx, boardno);				// 게시물번호				
				...
				pstmt.setInt(++idx, boardno);				// 시퀀스
				pstmt.setString(++idx, board_cd);			// 게시물코드					
				pstmt.setString(++idx, board_subj);			// 제목
				pstmt.setString(++idx, board_text);			// 내용				
				pstmt.setString(++idx, board_html_yn);		// html적용여부
				pstmt.setInt(++idx, refno);				// 참조번호
				pstmt.setLong(++idx, ansstg);				// 댓글단계수
				pstmt.setLong(++idx, anslev);				// 댓글레벨수
...
//					}
	 ...
	 * 답글 시퀀스 코드
	 */
	private StringBuffer getRoundingReplySeqQuery() throws Exception
	{
		StringBuffer sb = new StringBuffer();		
		sb.append(" SELECT NVL(MAX(ANS_STG),0)+1, NVL(MAX(ANS_LEV),0)+1 FROM BCDBA.TBGFBOARD WHERE REF_NO = ?	  \n");
	 ...
	 * 상위게시물의 스코어
	 */
	private StringBuffer getScorQuery() throws Exception
	{
		StringBuffer sb = new StringBuffer();
		sb.append("	SELECT NVL(SCOR_APPL_YN, 0) FROM BCDBA.TBGFBOARD WHERE BOARD_NO = ?	\n");
