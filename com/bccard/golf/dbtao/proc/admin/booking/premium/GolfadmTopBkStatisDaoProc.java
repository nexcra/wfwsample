...
*   클래스명  : GolfadmTopBkStatisDaoProc
*   작성자    : (주)미디어포스 이경희
...
public class GolfadmTopBkStatisDaoProc extends AbstractProc {
	...
	private String getSelectQuery(boolean dInq, String repMbNo, String objClss){
		...
		if (dInq) { sql.append("\nSUBSTR(A.TEOF_DATE, 1, 4)||'.'||SUBSTR(A.TEOF_DATE, 5, 2)  UNIT, 	   -- 라운딩일자                         "); }
	    else      { sql.append("\nSUBSTR(A.TEOF_DATE, 1, 4) AS UNIT,                                                			 "); }
		
        sql.append("\n            COUNT( DECODE(B.MEMBER_CLSS, '1', 'A', '4', 'A')) INDIV1 , --신청(회원구분코드  :1,4->개인)   ");        
        sql.append("\n       	 COUNT( DECODE(B.MEMBER_CLSS||A.PGRS_YN, '1B', 'A', '4B', 'A'))  INDIV2 , --성공 (회원구분코드, 부킹진행상태코드) ");
        sql.append("\n       	 COUNT( DECODE(B.MEMBER_CLSS||A.PGRS_YN, '1F', 'A', '4F', 'A'))  INDIV3 , --실패             	     ");
        sql.append("\n       	 COUNT( DECODE(B.MEMBER_CLSS||A.PGRS_YN, '1A', 'A', '4A', 'A', '1C', 'A', '4C', 'A'))  INDIV4 , --취소    ");
        sql.append("\n       	 COUNT( DECODE(B.MEMBER_CLSS, '5', 'A'))  CORP1 ,   --신청(회원구분코드  :5->법인)          	 ");
        sql.append("\n       	 COUNT( DECODE(B.MEMBER_CLSS||A.PGRS_YN, '5B', 'A'))  CORP2 ,    --성공                            			 ");
        sql.append("\n       	 COUNT( DECODE(B.MEMBER_CLSS||A.PGRS_YN, '5F', 'A'))  CORP3 ,    --실패                        			     ");
        sql.append("\n       	 COUNT( DECODE(B.MEMBER_CLSS||A.PGRS_YN, '5A', 'A', '5C', 'A')) CORP4	--취소				 ");
	 ...
	 ************************************************************************ */
	 private String getExcDetail(String memberClss, String gubun, String repMbNo){
	   ...
	   StringBuffer sql = new StringBuffer();
		...
		if(gubun.equals("2")){//성공
			sql.append("\n                 AND A.PGRS_YN = 'B'                                                    									");
	    }else if(gubun.equals("3")){//실패
		    sql.append("\n                 AND A.PGRS_YN = 'F'                                                     									");
	    }else if(gubun.equals("4")){//취소
