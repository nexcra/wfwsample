 ...
 * @version	1.0
 ******************************************************************************/
public class GolfadmParRsViewDaoProc extends AbstractProc {
	
	/** *****************************************************************
	 * GolfadmGrUpdFormDaoProc 프로세스 생성자
	 * @param N/A
	 ***************************************************************** */
	//public GolfadmParRsViewDaoProc() {}	
    ...
    ************************************************************************ */
    private String getSelectQuery(){
		...
		sql.append("\t 	, CASE T1.RSVT_YN WHEN 'Y' THEN '' WHEN 'D' THEN ''  										\n");
		sql.append("\t 	ELSE TO_CHAR(TO_DATE(SUBSTR(T1.CNCL_ATON, 0, 8)), 'YYYY-MM-DD (DY) ')  						\n");
		sql.append("\t	||SUBSTR(T1.CNCL_ATON, 9, 2)||':'||SUBSTR(T1.CNCL_ATON, 11, 2) END AS CNCL_DATE  			\n");		
		sql.append("\t 	, T6.APPR_OPION																				\n");
		sql.append("\t  , T3.GREEN_NM AS GR_NM, T1.CTNT AS CTNT														\n");
		sql.append("\t  , (TO_CHAR(TO_DATE(T1.ROUND_HOPE_DATE),'YY-MM-DD(DY)')) AS BK_DATE							\n");
		sql.append("\t  , T6.HG_NM AS NAME	,T6.EMAIL AS EMAIL1	,T6.PHONE	\n");
		sql.append("\t  , (SELECT COUNT(*) FROM BCDBA.TBGRSVTMGMT WHERE CDHD_ID=T1.CDHD_ID AND SUBSTR(GOLF_SVC_RSVT_NO,5,1)='P')AS TOTAL_CNT  \n");
