 ...
 * @version	1.0
 ******************************************************************************/
public class GolfadmParTimeXlsDaoProc extends AbstractProc {
	
	/** *****************************************************************
	 * GolfAdmLessonListDaoProc 프로세스 생성자
	 * @param N/A
	 ***************************************************************** */
	public GolfadmParTimeXlsDaoProc() {}	
    ...
    ************************************************************************ */
    private String getSelectQuery(String sch_GR_SEQ_NO, String sch_RESER_CODE, String sch_VIEW_YN, String sch_DATE, String sch_DATE_ST, String sch_DATE_ED){
		...
		sql.append("\n 		FROM (SELECT ROWNUM RNUM	");
		sql.append("\n 			, T1.TIME_SEQ_NO AS TIME_SEQ_NO	");
		sql.append("\n 			, (CASE WHEN T1.VIEW_YN='Y' THEN '노출' ELSE '비노출' END) VIEW_YN	");
		sql.append("\n 			, T3.GR_NM AS GR_NM	");
		sql.append("\n 			, T2.BKPS_DATE AS BKPS_DATE, T2.COURSE AS COURSE, T1.BKPS_TIME AS BKPS_TIME	");
		sql.append("\n 			, (CASE WHEN T1.RESER_CODE='0001' THEN '부킹대기' ELSE '부킹확정' END) RESER_CODE	");
		sql.append("\n 			, T2.REG_DATE AS REG_DATE	");
		sql.append("\n 			FROM 	");
		sql.append("\n 			BCDBA.TBBKTIME T1  	");
		sql.append("\n 			LEFT JOIN BCDBA.TBBKDAY T2 ON T1.DAY_SEQ_NO=T2.DAY_SEQ_NO 	");
		sql.append("\n 			LEFT JOIN BCDBA.TBBKGREEN T3 ON T2.GR_SEQ_NO=T3.SEQ_NO	");
		sql.append("\n 			WHERE T2.COURSE IS NOT NULL AND T2.BKPS_DATE IS NOT NULL AND T2.PAR_FREE IS NULL AND T2.SKY_CODE IS NULL	");
		...
		sql.append("\n 				ORDER BY T2.BKPS_DATE DESC, T1.BKPS_TIME DESC 	");		
		sql.append("\n 			)	");
		sql.append("\n 	ORDER BY RNUM	");
		sql.append("\n 	)	");
