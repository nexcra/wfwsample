...
*   클래스명  : GolfAdmPricesUpdFormDaoProc
*   작성자    : (주)만세커뮤니케이션 엄지환
*   내용      : 관리자 시세 수정
*   적용범위  : golf
*   작성일자  : 2009-07-06
 ...
 * @version	1.0
 ******************************************************************************/
public class GolfAdmPricesUpdFormDaoProc extends AbstractProc {
	
	/** *****************************************************************
	 * GolfAdmPricesUpdFormDaoProc 프로세스 생성자
	 * @param N/A
	 ***************************************************************** */
	public GolfAdmPricesUpdFormDaoProc() {}	
    ...
    ************************************************************************ */
    private String getSelectQuery(){
		...
		sql.append("\n 	NVL ((SELECT QUT FROM ");
		sql.append("\n 					(SELECT * FROM BCDBA.TBGGREENMEMRTKQUTMGMT	 ");
		sql.append("\n 						WHERE GREEN_MEMRTK_NM_SEQ_NO = ?	 ");
		sql.append("\n 						AND QUT_DATE <= ? AND QUT > 0 ORDER BY QUT_DATE DESC)	 ");
		sql.append("\n 		 WHERE ROWNUM < 2) , 0) QUT ");		
		sql.append("\n FROM BCDBA.TBGGREENMEMRTKMGMT TGM 	");
		sql.append("\n WHERE TGM.GREEN_MEMRTK_NM_SEQ_NO = ? 	");
